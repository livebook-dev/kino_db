...
export function init(ctx, info) {
  ...
  const SnowflakeForm = {
    ...
    computed: {
      isAuthSnowflake() {
        return this.fields.auth_type === "auth_snowflake" || "";
      },

      isAuthJwt() {
        return this.fields.auth_type === "auth_jwt";
      },
    },
    ...
    <div class="row">
      <BaseSelect
        ...
        v-model="fields.auth_type"
        selectClass="input input--xs"
        :grow
        :options="[
          { label: 'Password', value: 'auth_snowflake' },
          { label: 'JWT Private Key', value: 'auth_jwt' }
        ]"
      />
      <BaseSecret
        v-if="isAuthSnowflake"
        textInputName="password"
        secretInputName="password_secret"
        toggleInputName="use_password_secret"
        label="Password"
      ...
      <BaseSwitch
        v-if="isAuthJwt"
        name="use_encrypted_private_key"
        label="Encrypted"
        v-model="fields.use_encrypted_private_key"
      />
    </div>
    <div class="row">
      <BaseSecret
        v-if="isAuthJwt"
        textInputName="private_key"
        secretInputName="private_key_secret"
        toggleInputName="use_private_key_secret"
        label="JWT Private Key"
        v-model:textInputValue="fields.private_key"
        v-model:secretInputValue="fields.private_key_secret"
        v-model:toggleInputValue="fields.use_private_key_secret"
        modalTitle="Set JWT private key"
        :required
      />
      <BaseSecret
        v-if="isAuthJwt && fields.use_encrypted_private_key"
        textInputName="private_key_passphrase"
        secretInputName="private_key_passphrase_secret"
        toggleInputName="use_private_key_passphrase_secret"
        label="JWT Private Key Passphrase (Optional)"
        v-model:textInputValue="fields.private_key_passphrase"
        v-model:secretInputValue="fields.private_key_passphrase_secret"
        v-model:toggleInputValue="fields.use_private_key_passphrase_secret"
        modalTitle="Set JWT private key passphrase"
      />
    </div>
    `,
  };
