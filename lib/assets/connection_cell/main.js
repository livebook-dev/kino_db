...
export function init(ctx, info) {
  ...
  const DefaultSQLForm = {
    ...
    <div class="row">
      <BaseInput
        ...
        :grow
      />
      <div class="input-icon-container grow" v-if="fields.use_password_secret">
        <BaseSelect
      ...
      <label class="hidden-checkbox">
        ...
        />
        <span v-if="fields.use_password_secret" class="button--sm">
  ...
  const app = Vue.createApp({
    ...
    data() {
      return {
        ...
        availableSecrets: info.secrets.concat([
          { label: "New secret", value: "openSecrets" },
        ]),
        currentSecret: info.fields.password_secret,
      };
    },
    ...
    watch: {
      'fields.password_secret'(newSecret) {
    ...
    methods: {
      handleFieldChange(event) {
        ...
        if (field) {
          ...
          if (field === "password_secret" && value === "openSecrets") {
            this.fields[field] = this.currentSecret;
            ctx.openSecrets();
          } else {
  ...
  ctx.handleEvent("set_secrets", ({ secrets }) => {
    app.availableSecrets = secrets.concat([
      { label: "New secret", value: "openSecrets" },
    ]);
  });
